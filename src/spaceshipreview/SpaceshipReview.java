...
//Display the missiles.
        ...
        for (int i=0;i<missiles.length;i++)
        {
            if (missiles[i].active)
                g.setColor(Color.red);
                drawMissile(getX(missiles[i].xpos),getYNormal(missiles[i].ypos),
                0.0,0.3,0.3 );
        }        
...
//Display Force FIeld
        ...
        {
            g.setColor(Color.cyan);
            drawCircle(getX(rocketXPos),getYNormal(rocketYPos),0.0,3.0,3.0 );
        }
        
//Display the rocket.
        if(rocketXSpeed>0 || rocketXSpeed<0)
        {
            if (rocketRight)
                drawRocket(animRocketImage,getX(rocketXPos),getYNormal(rocketYPos),0.0,1.0,1.0 );
            else
                drawRocket(animRocketImage,getX(rocketXPos),getYNormal(rocketYPos),0.0,-1.0,1.0 );
        }
        ...
        {
            drawRocket(rocketImage,30+val,getY(-7),0.0,1.0,1.0 );
            val+=30;
        }
        
        g.setColor(Color.black);
        g.setFont(new Font("Helvetica",Font.BOLD,13));
        g.drawString("Score: " + score, 200, 45);
        
        g.setColor(Color.black);
        g.drawString("HIGH SCORE: "+highScore, getX(getWidth2()-95), 45);
        ...
        if (gameOver)
        {
            ...
            g.drawString("HIGH SCORE: "+highScore, 80, 250);
...
/////////////////////////////////////////////////////////////////////////
    public void animate() {
        if (animateFirstTime) {
            ...
            animRocketImage = Toolkit.getDefaultToolkit().getImage("./animRocket.GIF");
            reset();
        }
        ...
        try {
            ...
            while (line != null) {
                ...
                if (newLine.startsWith("nummissiles"))
                {
                    ...
                    Missile.numMissiles = Integer.parseInt(numMissilesString.trim());
                }
                line = in.readLine();
            }
